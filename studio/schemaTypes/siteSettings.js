...
export default defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      title: 'Logo',
      type: 'image',
      description: '建议透明 PNG/SVG，最长边 512 px 以上。',
    ...
    defineField({
      ...
      title: 'Favicon',
      type: 'image',
      description: '建议 256×256 px PNG（至少 64×64）。',
      options: { hotspot: true },
      group: 'brand',
    }),
    defineField({
      ...
      type: 'image',
      description:
        '已停用：请改为在「前端页面内容 → 首页 → 首页文案覆盖」配置实验室卡片配图。此处仅兼容旧数据。',
      options: { hotspot: true },
      hidden: true,
    }),
    defineField({
      name: 'coreCompetenceGmpcImage',
      title: '（遗留）首页 GMPC 卡片配图',
      type: 'image',
      description: '已停用：请在首页文档「首页文案覆盖」中配置 GMPC 配图。',
      options: { hotspot: true },
      hidden: true,
    }),
    defineField({
      ...
      title: '（遗留）研发卡片标题',
      type: 'string',
      description: '已停用：请在首页文档「首页文案覆盖」填写「研发卡片标题」。',
      hidden: true,
    }),
    defineField({
    ...
    defineField({
      ...
      title: '分享图（兼容旧字段）',
      type: 'image',
      description: '建议 1200×630 px（OG 标准）。',
      options: { hotspot: true },
      group: 'seo',
    }),
    defineField({
